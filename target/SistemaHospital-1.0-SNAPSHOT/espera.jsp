...
<head>
    ...
    <style>
        ...
        .btn-volver { padding: 12px 24px; background: var(--azul-oscuro); color: white; text-decoration: none; border-radius: 8px; font-weight: bold; transition: all 0.3s ease; letter-spacing: 1px; font-size: 14px; display: inline-block; text-align: center; }
        ...
        th { background: var(--azul-medio); color: white; padding: 20px 15px; text-align: left; font-size: 16px; letter-spacing: 1px; }
        ...
        .btn-edit { background: var(--teal-claro); color: var(--azul-oscuro); border: none; padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: 0.2s; font-size: 12px; letter-spacing: 0.5px; }
        .btn-edit:hover { background: var(--teal-oscuro); color: white; }
        
        .btn-rem { background: var(--morado-profundo); color: white; border: none; padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: 0.2s; margin-left: 8px; font-size: 12px; letter-spacing: 0.5px; }
        .btn-rem:hover { opacity: 0.9; transform: scale(1.05); }

        .btn-eliminar { background: #e74c3c; color: white; border: none; padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: 0.2s; margin-left: 8px; font-size: 12px; letter-spacing: 0.5px; }
    ...
    <div class="container">
        ...
        <table>
            ...
            <tr>
                ...
                <td><%= p.getCedula() %></td>
                <td>
                    <button class="btn-edit" onclick="mostrarEditar('<%= p.getCedula() %>','<%= p.getNombre() %>','<%= p.getApellido() %>')">editar</button>
                    <button class="btn-rem" onclick="mostrarRemision('<%= p.getCedula() %>')">remitir</button>
                    <form action="TriageServlet" method="POST" style="display:inline;">
        ...
        <form action="TriageServlet" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="sala">
                    ...
                    <option value="OBSERVACION">observacion (disp: <%= hosp != null ? hosp.getObservacionDisp() : 0 %>)</option>
                </select>
            </div>
            <div class="form-group">
            ...
            <button type="submit" class="btn-rem" style="width:100%; margin-top: 10px; background: var(--morado-profundo); font-size: 14px;">confirmar remision</button>
