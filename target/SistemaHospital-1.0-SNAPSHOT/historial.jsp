...
<html>
<head>
    <title>gestion de salas activas</title>
    <style>
        :root {
            --teal-claro: #5ab4ac;
            --teal-oscuro: #337a75;
        ...
        .btn-volver { padding: 12px 24px; background: var(--azul-oscuro); color: white; text-decoration: none; border-radius: 8px; font-weight: bold; transition: all 0.3s ease; letter-spacing: 1px; font-size: 14px; display: inline-block; }
        ...
        th { background: var(--morado-profundo); color: white; padding: 20px 15px; text-align: left; font-size: 16px; letter-spacing: 1px; }
        ...
        .btn-alta { background: var(--teal-claro); color: var(--azul-oscuro); border: none; padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: 0.2s; font-size: 12px; letter-spacing: 0.5px; }
        .btn-alta:hover { background: var(--teal-oscuro); color: white; transform: scale(1.05); }
        
        .btn-edit { background: #e2e8f0; color: var(--azul-oscuro); border: none; padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: 0.2s; font-size: 12px; letter-spacing: 0.5px; }
        .btn-edit:hover { background: #cbd5e1; transform: scale(1.05); }

        .btn-eliminar { background: #e74c3c; color: white; border: none; padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: 0.2s; font-size: 12px; letter-spacing: 0.5px; }
        .btn-eliminar:hover { opacity: 0.9; transform: scale(1.05); }
    ...
    <script>
        function mostrarEditar(c, n, a, est) {
            ...
            if(est && est !== "null") {
                document.getElementById('nEstado').value = est.toLowerCase();
            }
        }
...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <a href="index.jsp" class="btn-volver">volver al menu</a>
        </div>
        ...
        <table>
            ...
            <tr>
                <td><%= p.getCedula() %></td>
                <td style="text-transform: lowercase;"><strong><%= p.getNombreCompleto() %></strong></td>
                <td style="text-transform: lowercase;"><span class="badge-sala"><%= p.getSalaAsignada() != null ? p.getSalaAsignada() : "sin asignar" %></span></td>
                <td style="color: <%= (p.getEstadoClinico() != null && p.getEstadoClinico().toLowerCase().contains("critico")) ? "#e74c3c" : "var(--azul-oscuro)" %>; font-weight: bold; text-transform: lowercase;">
                    <%= p.getEstadoClinico() != null ? p.getEstadoClinico() : "no definido" %>
                </td>
                <td>
                    <div class="acciones-container">
                        <button class="btn-edit" onclick="mostrarEditar('<%= p.getCedula() %>','<%= p.getNombre() %>','<%= p.getApellido() %>','<%= p.getEstadoClinico() %>')">editar</button>
    ...
    <div id="editModal" class="modal">
        ...
        <form action="TriageServlet" method="POST">
            ...
            <div class="form-group">
                <label>nueva cedula:</label>
                <input type="text" name="cedula" id="nCed" required>
            </div>
            <div class="form-group">
                <label>estado clinico:</label>
                <select name="estado" id="nEstado">
                    <option value="critico (inestable)">critico (inestable)</option>
            ...
            <button type="submit" style="width:100%; margin-top: 10px; background: var(--teal-oscuro); color: white; border: none; padding: 12px; border-radius: 8px; font-weight: bold; cursor: pointer;">guardar cambios</button>
            <button type="button" onclick="cerrarModal('editModal')" style="width:100%; border:none; margin-top:10px; background: #94a3b8; color: white; padding: 12px; border-radius: 8px; font-weight: bold; cursor: pointer;">cancelar</button>
        </form>
    </div>
